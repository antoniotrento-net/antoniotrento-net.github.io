#!/usr/bin/env python3
"""Portfolio JPG/PNG da SVG ufficiali Qwibo (alta qualità, niente ridisegno manuale)."""
...
QWIBO_LOGO = Path(__file__).resolve().parents[2] / "qwibo" / "assets" / "images" / "logo"
...
TAGLINE_IT = "Trascrizione audio &amp; video · locale · privato"


def load_svg(name: str, *, replace_current_color: str | None = None) -> str:
    text = (QWIBO_LOGO / name).read_text(encoding="utf-8")
...
def build_index() -> None:
    mark = load_svg("logo-mark.svg", replace_current_color="#f0f4ff")
    w, h = INDEX_SIZE
    html = f"""<!DOCTYPE html>
...
<style>
  * {{ margin: 0; padding: 0; box-sizing: border-box; }}
  ...
  <div class="mark">{mark}</div>
    ...
    png_to_jpg(png, OUT / "qwibo-index.jpg")
...
<style>
  * {{ margin: 0; padding: 0; box-sizing: border-box; }}
  ...
  <p class="tagline">{TAGLINE_IT}</p>
    ...
    png = OUT / "_cover.png"
    render_html(html, w, h, png)
    png_to_jpg(png, OUT / "qwibo-cover.jpg")
    png.unlink(missing_ok=True)
...
<style>
  * {{ margin: 0; padding: 0; box-sizing: border-box; }}
  ...
  <div class="mark">{mark}</div>
    ...
    out = OUT / "qwibo-logo-1024x1024.png"
    render_html(html, size, size, out)


def main() -> int:
    if not QWIBO_LOGO.is_dir():
        raise SystemExit(f"SVG Qwibo non trovati: {QWIBO_LOGO}")
